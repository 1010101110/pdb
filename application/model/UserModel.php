...
class UserModel
{
    ...
    public static function getPublicProfilesOfAllUsers()
    {
        ...
        $sql = "SELECT user_id, user_name, user_email, user_bio, user_active, user_has_avatar, user_deleted, user_account_type FROM users";
        ...
        foreach ($query->fetchAll() as $user) {
            ...
            $all_users_profiles[$user->user_id]->user_bio = $user->user_bio;
    ...
    public static function getPublicProfileOfUser($user_id)
    {
        ...
        $sql = "SELECT user_id, user_name, user_email, user_active, user_has_avatar, user_deleted, user_bio, user_last_login_timestamp, user_creation_timestamp
    ...
    public static function getUserDataByUserNameOrEmail($user_name_or_email)
    {
        ...
        $query = $database->prepare("SELECT user_id, user_name, user_email, user_bio FROM users
    ...
    //save bio
    public static function saveBio($user_id, $newbio)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET user_bio = :bio WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':bio' => $newbio, ':user_id' => $user_id));
        if ($query->rowCount() == 1) {
            return true;
        }
        return false;
    ...
    //edits bio text
    public static function editBio($newbio)
    {
        // bio provided ?
        if (empty($newbio)) {
            Session::add('feedback_negative', "bio is empty");
            return false;
        }

        // check if new email is same like the old one
        if ($newbio == Session::get('bio')) {
            Session::add('feedback_negative', "bio is the same");
            return false;
        }
        
        // write to database, if successful ...
        // ... then write to session
        $status_of_action = self::saveBio(Session::get('user_id'), $newbio);
        if ($status_of_action) {
            Session::set('user_bio', $newbio);
            Session::add('feedback_positive', "updated bio");
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
    ...
    public static function getUserDataByUsername($user_name)
    {
        ...
        $sql = "SELECT user_id, user_name, user_email, user_password_hash, user_bio, user_active,user_deleted, user_suspension_timestamp, user_account_type,
    ...
    public static function getUserDataByUserIdAndToken($user_id, $token)
    {
        ...
        $query = $database->prepare("SELECT user_id, user_name, user_email, user_password_hash, user_active, user_bio,
