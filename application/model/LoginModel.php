...
class LoginModel
{
    ...
    public static function login($user_name, $user_password, $set_remember_me_cookie = null)
    {
        ...
        self::setSuccessfulLoginIntoSession(
            $result->user_id, $result->user_name, $result->user_email, $result->user_account_type, $result->user_bio
        );
    ...
    public static function loginWithCookie($cookie)
    {
        ...
        if ($result) {
            // successfully logged in, so we write all necessary data into the session and set "user_logged_in" to true
            self::setSuccessfulLoginIntoSession($result->user_id, $result->user_name, $result->user_email, $result->user_account_type, $result->user_bio);
            // save timestamp of this login in the database line of that user
            self::saveTimestampOfLoginOfUser($result->user_name);
     ...
     */
    public static function setSuccessfulLoginIntoSession($user_id, $user_name, $user_email, $user_account_type,$user_bio)
    {
        Session::init();
    ...
    public static function saveTimestampOfLoginOfUser($user_name)
    {
        ...
        $sql = "UPDATE users SET user_last_login_timestamp = now()
